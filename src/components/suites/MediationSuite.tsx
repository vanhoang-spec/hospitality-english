...
function MediationSuiteInner({
  ...
  function submit() {
    ...
    recordSuiteResult(dep, week, "mediation", r.passed ? 8 : 0, {
      scorePct: r.scorePct,
      mastered: r.passed,
    });
  }
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
      ...
      >
        <div className="border border-primary/30 bg-card p-6 shadow-xl">
          <div className="text-[10px] uppercase tracking-[0.3em] text-primary">
            Lời bạn nói với khách (tiếng Anh)
          </div>
          <textarea
            value={draft}
            disabled={submitted}
              ...
              <div className="border border-primary bg-background/40 p-4 text-center">
                ...
                {scorePct >= PASS_PCT ? (
                  ...
                  <div className="mt-1 text-[11px] text-foreground/60">
                    {blockedVi ??
                      `Cần ≥ ${PASS_PCT}% số ý. Xem gợi ý bên dưới rồi sửa lại câu của bạn.`}
                  </div>
                )}
              </div>
