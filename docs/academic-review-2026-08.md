# Đánh giá học thuật toàn khoá — 40 tuần × 6 bộ phận

**Vai trò rà soát:** Academic Director (giảng dạy & thiết kế chương trình tiếng Anh nghiệp vụ khách sạn/resort 4–5 sao tại Việt Nam)
**Ngày:** 2026-08-01
**Phạm vi:** toàn bộ 240 dep-week (6 bộ phận × 40 tuần), 960 bài học, cùng cơ chế đo lường và cơ chế tạo động lực trong app.
**Chuẩn đối chiếu:** CEFR A2 cho giao tiếp nghiệp vụ khách sạn/nhà hàng là **chuẩn đầu ra bắt buộc ở tuần 40**. Phần app tự gắn nhãn B1.1 (tuần 31–40) được xét riêng như phần mở rộng, kèm câu hỏi: nhãn đó có tương xứng với nội dung thực tế không.

---

## 1. Kết luận điều hành

**Khung chương trình xuất sắc. Nội dung thực thi chưa đạt để ký duyệt.**

Đây là một thiết kế chương trình đáng nể ở tầng khung: năm phase có mốc CEFR rõ, trần độ dài câu được tuân thủ 100% (0 vi phạm trên 4.206 câu đích), bài sát hạch có sàn điểm từng kỹ năng và có phần thi nói bắt buộc, hệ thống ôn tập ngắt quãng dùng khoá bền vững, và toàn bộ giải thích ngữ pháp bằng tiếng Việt. Rất ít sản phẩm ESP thương mại ở Việt Nam có nền móng học thuật chặt như vậy.
...
**Khuyến nghị tổng:** Không triển khai thương mại phần tuần 15–40 ở trạng thái hiện tại. Cần một đợt sửa nội dung tập trung vào **khung câu (frame), không phải từng câu** — vì mỗi khung hỏng nhân ra 4–5 lỗi và lan sang nhiều bộ phận. Chi tiết ưu tiên nằm ở `docs/academic-review-backlog.md`.
...
### 3.2 Ba thang độ khó bị gãy

**Bài đọc đứng yên hai chặng.** Độ dài trung bình: 19,5 → 19,9 → 26,6 → 38,3 → 39,5 từ. P0→P1 tăng 2%, P3→P4 tăng 3%. Mười tuần cuối — đoạn dán nhãn B1.1 — bài đọc thực tế không dài hơn phase trước.

**Tốc độ nghe không hề tăng.** `docs/curriculum-level-matrix.md:108` cam kết thang 0,7 → 0,75 → 0,8 → 0,85–0,9 → 0,9. Nhưng `src/components/suites/ListeningSuite.tsx:44` đặt cứng `u.rate = 0.8 + Math.random() * 0.2` cho **mọi tuần**. Người mới học tuần 1 nghe cùng tốc độ với người tuần 40. Thang nghe được ghi trong tài liệu nhưng chưa từng được lập trình.

**Tổng từ vựng dưới mục tiêu.** Thực tế 504–508 từ/bộ phận, so với mục tiêu 560–620 từ của matrix (dòng 112) — hụt khoảng 10%.

### 3.3 Vấn đề nghiêm trọng nhất: tái tạo thay vì sản sinh

Đây là điểm quyết định verdict A2.

- **1.296/1.368 (94,7%)** speaking item được chấm bằng `utterancePassed()` so với **một câu mẫu cố định**, ngưỡng 60–80% từ khớp cộng thứ tự LCS 0,4–0,6 (`src/lib/speaking-score.ts:86-91`). Đây là đo độ chính xác nhắc lại, không phải đo năng lực nói.
- Chỉ **72 item (5,3%)**, toàn bộ ở tuần 39–40, dùng ngưỡng mở (50%, không ràng buộc thứ tự).
...
**Kết luận vòng 1:** nội dung ngôn ngữ đủ tầm A2 từ khoảng tuần 22–26. Nhãn B1.1 cho tuần 31–40 không được chứng minh: bài đọc không dài hơn, nghe không nhanh hơn, viết không được đo, và mật độ lỗi cao nhất nằm ở đây.

---

## 4. Vòng 2 — Chất lượng ngôn ngữ của nội dung sinh tự động

### 4.1 Cơ chế sinh lỗi

228/240 tuần được tạo bằng `buildPhase1..4()`: một bộ khung câu dùng chung, nhét từ vựng riêng của từng bộ phận vào ô trống. **Không có kiểm tra loại ngữ nghĩa nào** — một danh từ chỉ tài liệu bị nhét vào ô cần danh từ chỉ sự cố, một danh từ chỉ khoản phí bị nhét vào ô cần khoảng thời gian, và câu vẫn được xuất ra như câu mẫu đúng.

Nghiêm trọng hơn: **cùng một câu hỏng được chép vào 4–5 chỗ** (ngữ cảnh từ vựng, câu `polite` mẫu, câu đích luyện nói, bài đọc, đáp án đúng của trò chơi). Một ô điền sai đẻ ra 4–5 lỗi mà học viên gặp.
...
### 4.3 Lỗi nguy hiểm nhất: bài học xử lý khủng hoảng

Tuần 39, bài "Under Pressure" (`src/lib/content/phase4.ts:2354-2357`). Câu khách nói được viết cứng:

> **Khách:** "There is smoke in the corridor and our guests are panicking!"
...
Tương tự ở tuần 36 (GR): khách hỏi "Is it finally over? Can we go back up?" sau sự cố sơ tán → đáp mẫu "The **flight cancellation** has been fully resolved."

### 4.4 Lỗi ngữ pháp trong câu được đánh dấu là ĐÚNG

Cần nói rõ: trường `grammar.rude` là câu sai **cố ý** (lỗi mẫu của người Việt) — không tính. Nhưng những câu dưới đây nằm ở `grammar.polite`, tức **câu mẫu đúng học viên phải ghép lại và ghi nhớ**:

- Tuần 11, cả 5 bộ phận: "He **make the beds the room** every day." (HK) · "He **meet the guests the room** every day." (GR) · "He **massages the room** every day." (SW) · "He **check ins the room** every day." (FO) — vừa sai chia động từ ngôi ba, vừa thừa tân ngữ.
- BO-21: "**It checked the figures than usual**, sir." (×4 đơn vị) · "**I signed the supervisor** this afternoon." (×4)
- SW-18: "A ten percent **duration** is added, madam." (×5 — ô "phí phục vụ" bị điền bằng từ "thời lượng", và đáp án tiếng Việt trong bài quiz cũng lan lỗi thành "Thời lượng") · "One moment, please. **The system is confirmed**." (×3)
- GR-20: "Would you prefer **the lounge or room or the early or late**?" (×3) · bài đọc: "It would **relaxing option** nicely."
- SW-29: "Everything has been recorded in the **locker key count**." (×5)
- FB-13: "The **pipe is overcooked**." · "The **machine is unhappy**."
- HK-12: "**Let me send up for you.**" (thiếu tân ngữ, xuất hiện 2 lần dưới dạng câu mẫu)
- Tuần 10: "This one is **more empty**." · "This one is **more bright**." · "It is **a little safe**."

### 4.5 Lỗi đại từ theo nhân vật

FB/SW/GR dùng nhân vật nữ (Linh, Mai, Trang) nhưng khung câu vẫn xuất đại từ nam: **57 câu** kiểu "Linh shows **his** log", "Mai answers… **He** asks". Nặng nhất là tuần 7 của FB, dạy nguyên văn:
...
## 5. Vòng 3 — Tính đặc thù nghề nghiệp và độ xác thực

### 5.1 Bốn tuần đầu dạy sai nghề

Chính `verify-content.ts` báo cáo: tuần 1 chỉ 10% nội dung đặc thù bộ phận, tuần 2 40%, và **tuần 3, 4, 5, 6 là 0%**. Đo độc lập (câu đích trùng nhau giữa ≥2 bộ phận): tuần 5 trùng 100%, tuần 4 trùng 92%.

Cụ thể tuần 4, nhân viên buồng phòng, spa và back-office đều học y hệt nhau:

> "Cash or card, sir?" · "It is twenty dollars, sir." · "The price is twenty dollars."

Nhân viên buồng phòng không thu tiền khách. Và giá được yết bằng **đô la Mỹ** — trong khi giao dịch tại Việt Nam phải yết bằng VND. Tuần 4 có dạy "Five hundred thousand dong" và "Do you take dong?" (tốt), nhưng câu mẫu học viên nhại theo lại là "Two coffees. Eight dollars, please."

Từ tuần 7 trở đi độ đặc thù lên 61–89% — hoàn toàn ổn. Vấn đề chỉ nằm ở sáu tuần đầu, tức đúng giai đoạn quyết định người học có ở lại hay không.

### 5.2 Vách chất lượng giữa tuần soạn tay và tuần sinh tự động
...
Ngược lại, 5/6 bộ phận đều bị "tặng" tuần 37–38 về hợp đồng và thuyết trình đề xuất — nội dung B2B mà nhân viên buồng phòng hay kỹ thuật viên spa sẽ không bao giờ dùng. Đây là **hai tuần lãng phí × 5 bộ phận = 10 tuần** có thể dùng để lấp các khoảng trống ở trên.

### 5.4 Thực tế Việt Nam

- **Tiền tệ mâu thuẫn:** tuần 4 dạy giao dịch bằng USD ở cả 6 bộ phận; HK-33 thương lượng đền bù "$80 / $20". Trong khi FO-17 lại rất đúng: "security deposit of 1,000,000 VND per night". Không có Momo/VNPay/QR — cách thanh toán của một nửa số khách hiện nay.
- **Tết chỉ xuất hiện ở BO-37** (và ở đó thì rất chuẩn: blackout 15/01–05/02 và 30/04–03/05). Các bộ phận tiếp khách không có nội dung Tết nào.
- **Mùa bão: đúng một lần** trong 240 tuần (bản tin cờ đỏ của SW-19). FO và GR không có gì.
- **Không có chiến lược giao tiếp với khách hạn chế tiếng Anh.** Mọi vị khách trong giáo trình đều nói tiếng Anh trôi chảy, chuẩn. Thực tế Nha Trang (khách Nga), Đà Nẵng (khách Hàn/Trung) hoàn toàn ngược lại — và đó là thách thức hằng ngày lớn nhất.
- **Không có địa danh nào.** Giáo trình cố ý trung lập về địa điểm, nhưng điều đó làm rỗng toàn bộ mảng concierge/GR.

---

## 6. Vòng 4 — Thời lượng thực tế và hành trình người học

### 6.1 Nhãn "4 giờ mỗi tuần" sai gấp 5 lần
...
Đây là con dao hai lưỡi. Về mặt giữ chân người học, 46 phút/tuần là **tin tốt** — hoàn toàn khả thi với nhân viên ca gãy. Nhưng nhãn "4h" khiến người học bận rộn nhìn vào là nản ngay từ đầu, và 31 giờ học **không đủ** để đưa một người mất gốc lên A2 vững; chuẩn ngành cho pre-A1 → A2 là khoảng 180–200 giờ có hướng dẫn. Nói cách khác: app hứa 160 giờ, hiển thị 4 giờ/tuần, và giao 46 phút.

### 6.2 Ma sát khi quay lại app

Với người học có 10–15 phút rảnh, đường đi đến bài học là: trang chủ → chọn bộ phận → cuộn qua timeline 40 tuần → tìm tuần đang học → chọn suite. **Không có nút "Học tiếp".** Và quan trọng hơn:

- `Tier3SkillSuitesHub.tsx` **không đọc `lesson_progress`** — màn hình tuần không đánh dấu suite nào đã hoàn thành. Người học quay lại sau 4 ngày phải tự nhớ mình đã học Vocab và Grammar chưa.
- `department.$dep.tsx` chỉ hiện trạng thái khoá/mở và tên phase, không đánh dấu tuần đã xong.
- `lesson_progress` chỉ được đọc bởi `week-access.ts` (để mở khoá) và `org-admin.tsx` (cho quản lý). **Người học không có bất kỳ màn hình "N/40 tuần" nào.** Quản lý nhìn thấy tiến độ của nhân viên; nhân viên thì không.
- Không có điểm lưu giữa chừng: trạng thái suite chỉ nằm trong React state. Mất sóng, khoá màn hình, hay có khách gọi là mất cả buổi học. Ngoại lệ duy nhất là phần viết của bài sát hạch (`WeekTestSuite.tsx:496`).

### 6.3 Ba ngưỡng "đạt chuẩn" mà người mất gốc không thể chạm tới

Đây là phát hiện nghiêm trọng nhất về trải nghiệm, vì nó giết đúng nhóm đông nhất trong hai tuần đầu.

**Từ vựng tuần 1.** HK tuần 1 có 10 từ, chưa có từ ôn. Bài quiz vì vậy gồm 10 câu trắc nghiệm + 3 câu chính tả nghe-gõ = 13 câu. `MASTERY_PCT = 80` trên 13 câu đòi **11 câu đúng**. Người học mất gốc gõ được từ tiếng Anh nghe qua loa điện thoại là chuyện không thể — và chấm chính tả là so khớp chuỗi tuyệt đối (`VocabSuite.tsx:161`: `typed.trim().toLowerCase() === q.word.toLowerCase()`), không dung sai một ký tự. Vậy **đúng trọn 10/10 câu trắc nghiệm mà trượt cả 3 câu chính tả = 76,9% = KHÔNG ĐẠT.** Buổi học đầu tiên của người học kết thúc bằng một màn hình báo trượt, dù họ đã làm đúng mọi câu trong khả năng.

**Nghe tuần 1.** Sinh ra 4 câu chọn đáp án + 4 câu điền từ nghe được. Câu điền chấm theo `task.tokens.every(...)` (`ListeningSuite.tsx:139-141`) — **đúng hết hoặc không điểm**, không có điểm thành phần, dù một câu có tới 3 chỗ trống. Trần thực tế của người mới học là 4/8 = 50%, trong khi chuẩn đòi 7/8.
...
`GrammarSuite.next()` chỉ tăng `round` vô hạn, với `puzzleIdx = round % puzzles.length` (`GrammarSuite.tsx:62,161-163`) — giải xong câu cuối, tiêu đề lặng lẽ quay về "Câu 1/9". `SpeakingSuite` cũng vậy: `setIdx((i) => (i + 1) % scenarios.length)`, không có trạng thái `done`. Hai trong sáu suite, lặp lại suốt 40 tuần, không bao giờ cho người học một khoảnh khắc "xong rồi".
...
### 6.6 Người dùng điện thoại mất động lực dài hạn

`AcademyNav.tsx` đặt khối "Your Career Growth" (thứ bậc nghề nghiệp, 9 cấp từ Trainee đến General Manager) trong `hidden … md:flex`, và link Appraisal trong `hidden … md:inline-flex`. Dưới 768px — tức **toàn bộ người dùng điện thoại, chính là đối tượng mục tiêu** — không bao giờ nhìn thấy cấp bậc nghề nghiệp của mình lẫn bảng đánh giá năng lực. Chỉ còn hai huy hiệu sao và chuỗi ngày.

Thứ bậc nghề nghiệp là đòn bẩy động lực dài hạn mạnh nhất mà app đã xây xong — và nó bị ẩn khỏi đúng nhóm cần nó nhất.

---

## 7. Vòng 5 — Hệ thống tạo động lực
...
8. **Sao có thể "cày" được ở bốn trên sáu suite**, làm hỏng cả thang cấp bậc lẫn bảng theo dõi của quản lý (`org-admin.tsx` hiển thị `service_stars` từng nhân viên):
   ...
   - `ArcadeSuite.tsx:100,114-121` — `awardStars(2)` mỗi bong bóng đúng, chặn trùng bằng `poppedRef` khoá theo `id: ++idRef.current` là bộ đếm theo mỗi lần mount. Chơi lại là sinh id mới, thưởng lại từ đầu.
   - `ReadingSuite.tsx:64-68` — `awardedPassageRef` chỉ lưu **một** chỉ số cuối cùng, nên đổi qua lại giữa hai bài đọc là được thưởng lại cả hai.

   Đạt cấp "General Manager" (3.600 sao) bằng cách cày Arcade mất khoảng vài giờ chạm màn hình và **không đòi hỏi biết tiếng Anh**.

9. **Bảng đánh giá năng lực `/appraisal` đo nhiễu, không đo năng lực.** `GrammarSuite.tsx:131` đặt `courtesy_score = min(100, 70 + (round + 1) * 8)` — một hàm của **số vòng đã bấm**, không phải của độ đúng; nó chạm trần 100 sau câu đố thứ tư rồi đứng yên suốt 40 tuần. `SpeakingSuite` ghi đè `fluency_score` bằng **lần thử gần nhất** chứ không phải lần tốt nhất, nên luyện thêm làm biểu đồ tụt xuống. Nếu quản lý dùng bảng này để ra quyết định nhân sự, họ đang đọc số liệu vô nghĩa.

---

## 8. Vòng 6 — Giá trị đo lường

### 8.1 Thiết kế bài sát hạch: chắc chắn

Bài sát hạch ở tuần 6/14/22/30/40 là phần được thiết kế tốt nhất của hệ thống:

- 20 câu viết (từ vựng 8 / ngữ pháp 4 / nghe 4 / đọc 4), cần ≥70% tổng **và** ≥50% mỗi khối kỹ năng (`phases.ts:37-84`). Sàn từng kỹ năng bịt đúng lỗ hổng nguy hiểm: trước đó có thể đạt 80% tổng với 0/4 điểm nghe.
- 5 câu nói, phải đạt 3 (`CHECKPOINT_ORAL_ITEMS`, `CHECKPOINT_ORAL_PASS_MIN`). Không còn chuyện lên B1.1 mà chưa từng mở miệng.
- Nghỉ 20 phút sau khi trượt — chống quay xổ số, nhưng đủ ngắn để người làm ca thử lại trong cùng buổi.
- Hồ đề đủ lớn để không học tủ: phase 4 của FO có 168 mục từ vựng, 81 ngữ pháp, 67 câu nói, 80 câu hỏi đọc.
- Có cơ chế miễn sàn nghe khi thiết bị không có giọng đọc tiếng Anh — xử lý đúng thực tế điện thoại Android giá rẻ và WebView của Zalo/Facebook.

### 8.2 Ba lỗ hổng đo lường
...
**Không đo kỹ năng viết.** Bài sát hạch chỉ có từ vựng/ngữ pháp/nghe/đọc + nói. Kỹ năng viết chỉ xuất hiện ở tuần 33 và không nằm trong bất kỳ bài kiểm tra nào.

**Tuần 40 không phải bài thi cuối khoá.** `curriculum-level-matrix.md:250` hứa "Đánh giá cuối khoá — mock role-play + weektest toàn lộ trình". Nội dung thực tế là bốn bài tự phản ánh ("Everything You Can Now Do", "Forty Weeks Complete") với câu mẫu chính là các khung câu hỏng đã nêu ở §4.2 ("The departure transfer taught me the most"). Cửa gác thực tế vẫn là bài 20 câu + 5 câu nói giống hệt tuần 6/14/22/30 — chỉ khác hồ đề.

### 8.3 Chấm nói: hợp lý trong giới hạn công nghệ

`speaking-score.ts` chấm bằng so khớp túi từ **cộng** tỉ lệ đúng thứ tự qua LCS — chặn được mẹo đọc đúng từ nhưng sai trật tự. Ngưỡng giãn theo tuần và luôn chấm theo tuần gốc của câu, nên bài thi cuối không thành bài dễ nhất. Có đường lùi gõ phím khi micro lỗi.

Giới hạn cần nói thẳng: **không có đánh giá phát âm ở mức âm vị.** IPA được hiển thị trên thẻ từ nhưng không bao giờ được chấm. Với người Việt, các lỗi cố hữu — nuốt âm cuối /s/, /t/, /k/, lẫn /l/–/n/ — là thứ khiến khách nước ngoài không hiểu, và hệ thống hiện tại không phát hiện được. Chính `curriculum-level-matrix.md` đặt trọng tâm phát âm âm cuối cho phase 0 nhưng không có cơ chế đo nào.

### 8.4 Ghép câu bằng chip quá dài trên điện thoại

232/1.943 câu ngữ pháp (12%) tạo ra hơn 12 chip; 112 câu (6%) từ 15 chip trở lên; dài nhất là HK-33 với 23 chip. Ghép 23 mảnh từ trên màn hình điện thoại không còn là bài tập ngữ pháp mà là bài kiểm tra kiên nhẫn. `qa-full.ts` đã cảnh báo 14 trường hợp nhưng cảnh báo chưa được xử lý.

---

## 9. Dự báo tỉ lệ hoàn thành
...
## 11. Trả lời trực tiếp hai câu hỏi của đề bài

### 11.1 App có đưa người học từ số 0 đến hết A2 giao tiếp khách sạn không?

**Chưa, ở trạng thái hiện tại.** Nhưng khoảng cách là khoảng cách thực thi, không phải khoảng cách thiết kế.

Nội dung ngôn ngữ đã đủ tầm A2 từ khoảng **tuần 22–26**: các thì, modal, so sánh, câu điều kiện, chức năng dịch vụ lịch sự đều có mặt đúng thứ tự. Nếu 228 tuần sinh tự động đạt chất lượng của 12 tuần soạn tay, tôi sẽ ký duyệt đây là một lộ trình A2 tốt.

Ba rào cản khiến hiện tại chưa đạt:

1. **23% câu mẫu có lỗi** ở nội dung sinh tự động. Người học ghi nhớ chính những câu này. Học thuộc "He make the beds the room every day" và "I am ready for the stay summary" không đưa ai tới A2 — nó tạo ra lỗi hoá thạch phải mất công gỡ sau này.
2. **Huấn luyện tái tạo, không phải sản sinh.** 94,7% hoạt động nói là đọc lại câu mẫu; 3,1% là đặt câu hỏi. A2 được định nghĩa bằng khả năng trao đổi thông tin trong tình huống quen thuộc, mà đó chính là phần không được luyện.
3. **~31 giờ học thật** so với chuẩn ngành 180–200 giờ cho chặng pre-A1 → A2. Thời lượng chỉ bằng một phần sáu.

Đánh giá của tôi: người hoàn thành đủ 40 tuần ở trạng thái hiện tại sẽ đạt khoảng **A1+ đến A2.1 trong phạm vi kịch bản đã học thuộc**, và sụt xuống dưới mức đó ngay khi khách nói lệch kịch bản. Sau khi sửa xong nhóm lỗi P0 và bổ sung luyện sản sinh, mốc A2 ở tuần 40 là khả thi.

### 11.2 Nhãn B1.1 cho tuần 31–40 có xứng đáng không?

**Không.** Câu dài hơn và cú pháp phức tạp hơn thì có thật, nhưng: bài đọc không dài hơn phase 3 (+3%), tốc độ nghe không tăng suốt cả khoá, viết không được đo trong bất kỳ bài sát hạch nào, và đây là đoạn có mật độ lỗi cao nhất (26–37%). Nên gỡ nhãn B1.1, đặt lại tuần 31–40 là **"A2 vững / A2+ mở rộng nghiệp vụ"**, và giữ B1.1 làm mục tiêu cho một module sau khi chất lượng nội dung đạt chuẩn.

### 11.3 App có giữ được động lực cho người đi làm bận rộn không?

**Chưa — và nguyên nhân không nằm ở chỗ thiếu tính năng, mà ở chỗ những tính năng đã xây không kết nối với đời sống của người học.**

App đã có gần đủ nguyên liệu đúng: chuỗi ngày học thưởng nỗ lực chứ không phạt sự yếu kém, ôn tập ngắt quãng đúng khoa học, thứ bậc nghề nghiệp gắn với thăng tiến thật, phiên ôn ngắn vài phút, sổ tay in được. Đây là những lựa chọn của người hiểu việc.

Nhưng: **không có gì nhắc người học quay lại.** Chuỗi ngày và hàng đợi ôn tập chỉ có ý nghĩa nếu người học nhớ mở app — mà nhân viên khách sạn làm ca 6 ngày/tuần thì không nhớ. **Không có gì chạy được khi mất mạng.** **Không có gì cho thấy đã đi được bao xa** — trong khi quản lý thì nhìn thấy đầy đủ. Và trên điện thoại, thứ bậc nghề nghiệp — đòn bẩy mạnh nhất — bị ẩn đi.

Sáu tuần đầu làm mọi thứ tệ hơn: không phân hoá theo bộ phận (tuần 3–6 là 0%), nên nhân viên buồng phòng bỏ 4 tuần học cách thu tiền cà phê bằng đô la trước khi gặp bài học đầu tiên liên quan đến công việc của mình. Đó là đúng giai đoạn quyết định người học ở lại hay bỏ.

Chẩn đoán của tôi: khoảng cách giữa app hiện tại và một app thực sự giữ chân người học là **bốn thay đổi** — nhắc học, chạy offline, nút "học tiếp" kèm chỉ dấu tiến độ, và phân hoá nghề nghiệp ngay từ tuần 1. Không cái nào đòi hỏi viết lại kiến trúc.

Và một điều nữa cần nói thẳng với ban giám đốc: **can thiệp hiệu quả nhất trên mỗi đồng bỏ ra không nằm trong phần mềm.** Bảng theo dõi dành cho quản lý (`org-admin.tsx`) đã đủ tốt để một trưởng bộ phận rà tiến độ hằng tuần, đặt mục tiêu và nhắc nhân viên trực tiếp — và chỉ riêng việc đó, theo dự báo ở §9, đã nhân đôi tỉ lệ hoàn thành ở mọi mốc mà không cần sửa dòng code nào. Một chương trình đào tạo tại doanh nghiệp thành công hay thất bại ở chỗ có người theo dõi hay không, chứ không ở chỗ app có bao nhiêu huy hiệu.

---

## 12. Ưu tiên hành động

Danh sách chi tiết kèm vị trí file nằm ở **`docs/academic-review-backlog.md`**. Thứ tự chiến lược:

**Đợt 1 — chặn dạy sai (bắt buộc trước khi triển khai bất kỳ tuần nào ≥15):**
Sửa các khung câu hỏng ở tầng generator (không sửa từng câu — mỗi khung nhân ra 4–5 lỗi và lan sang nhiều bộ phận). Bắt đầu từ khung khủng hoảng tuần 36/39, khung `Based on your {X}`, khung `I am ready for the {X}`, lỗi chia động từ tuần 11, và lỗi đại từ nhân vật nữ.

**Đợt 2 — bổ sung gate ngữ nghĩa:**
QA hiện tại không thể bắt loại lỗi này. Cần thêm một tầng kiểm tra nghĩa (ràng buộc loại ngữ nghĩa cho từng ô điền trong bank contract, cộng rà soát bằng mô hình ngôn ngữ cho toàn bộ câu sinh tự động) trước khi sinh thêm bất kỳ nội dung nào.

**Đợt 3 — đóng khoảng cách A2:**
Tăng tỉ lệ luyện sản sinh (đặt câu hỏi, hội thoại nhiều lượt, viết), sửa lỗi bỏ sót từ mới trong quiz, lập trình thang tốc độ nghe, phân hoá tuần 1–6 theo bộ phận.

**Đợt 4 — giữ chân người học:**
Nhắc học, PWA/offline, nút "học tiếp" + chỉ dấu hoàn thành, hiện thứ bậc nghề nghiệp trên mobile, bài kiểm tra xếp lớp.
